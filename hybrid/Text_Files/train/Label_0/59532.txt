...
public class Main {
	...
	void solve (FastScanner in, PrintWriter out, Methods ms) {
		...
		for (int i=0; i<n; i++) bigList.add(new ArrayList<>());
		...
		for (int i=0; i<q; i++) {
			...
			if (mode == 0) {
				bigList.get(num).add(in.nextInt());
			}
			else if (mode == 1) {
				printList(out, bigList.get(num));
			}
			else if (mode == 2) {
				bigList.get(num).clear();
			}
	...
	static void printList (PrintWriter out, ArrayList<Integer> list) {
