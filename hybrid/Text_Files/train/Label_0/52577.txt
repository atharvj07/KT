...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		long[][][] xys = new long[n][2][2]; //xy,sg
		
		Set<Long> px = new TreeSet<>();
		Set<Long> py = new TreeSet<>();
		for(int i=0; i<n; i++){
		    xys[i][0][0]=sc.nexL();
		    xys[i][1][0]=sc.nexL();
		    xys[i][0][1]=sc.nexL();
		...
		Long[] px2 = px.toArray(new Long[0]);
		Long[] py2 = py.toArray(new Long[0]);
		
		int kx = px2.length;
		int ky = py2.length;
		
		
		long[][] seach = new long[ky][kx];
		for(int i=1; i<ky; i++){
		    for(int j=1; j<kx; j++){
		        seach[i-1][j-1] = (px2[j]-px2[j-1])*(py2[i]-py2[i-1]);
		    }
		    seach[i-1][kx-1]=0;
		}
		for(int j=0; j<kx; j++){
		...
		int[][] acum = new int[ky][kx]; //2次元imos
		fill(acum,0);
		
		for(int i=0; i<n; i++){
		    int sx = biSearch(px2, xys[i][0][0]);
		    int gx = biSearch(px2, xys[i][0][1]);
		    int sy = biSearch(py2, xys[i][1][0]);
		    int gy = biSearch(py2, xys[i][1][1]);
		    acum[sy][sx]++;
		    acum[gy][sx]--;
		    acum[sy][gx]--;
		    acum[gy][gx]++;
