...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        out.println(chinesePostman(g));
        
        out.flush();
    }
    
	static int chinesePostman(WGraph g){
		int n = g.vnum();
		ArrayList<Integer> odds = new ArrayList<>();	//奇点
		int[][] dist = g.WarshallFloyd();
		int total = g.totalWeight();
			...
			for(int B = 0; B<S; B++){
				for(int i=0; i<k; i++){
					...
					for(int j=i+1; j<k; j++){
						...
						if(dist[odds.get(i)][odds.get(j)] == Integer.MAX_VALUE){
							continue;
						}
						if(dp[B] == Integer.MAX_VALUE){
							continue;
						}
						int newstate = B|(1<<i)|(1<<j);
						dp[newstate] = Math.min(dp[newstate], dp[B] + dist[odds.get(i)][odds.get(j)]);
					}
				}
			}
