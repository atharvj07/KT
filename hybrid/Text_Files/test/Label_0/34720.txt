	...
	void solve() throws IOException{
		...
		for(int i=1; i<h; i++){
			out:for(int j=0; j<w; j++){
				if(map[i][j] != '-') continue;
				for(int d=0; d<2; d++){
					final int ny = i+dy1[d];
					final int nx = j+dx1[d];
					if(!out(ny, nx) && map[ny][nx]=='o') continue out;
				}
				for(int d=0; d<8; d++){
					final int ny = i+dy2[d];
					final int nx = j+dx2[d];
					if(!out(ny, nx) && map[ny][nx]=='x') continue out;
				}
				cnt++;
			}
		}
